...
export default function FullTestPage() {
    ...
    const generateDetailedLog = (score) => `<h3>Số câu đúng: ${score}</h3>`;
    ...
    const handleUnifiedGrading = async () => {
        ...
        setTimeout(() => { setIsGrading(false); toast.success("✅ Đã nhận phản hồi!"); }, 2000);
    };
    ...
    useEffect(() => {
        ...
        if (canTick && timeLeft > 0 && !loadingData) {
            timer = setInterval(() => {
                setTimeLeft(prev => {
                    if (prev <= 1) { clearInterval(timer); handleSubmitAuto(); return 0; }
                    return prev - 1;
                });
            }, 1000);
        }
        return () => clearInterval(timer);
    }, [isTestStarted, isAudioPlaying, timeLeft, loadingData, skill]);
    ...
    const sendEmailReport = (score, total, type, detailedLog = "") => {
        ...
        if (type === 'lr') {
            ...
            templateParams = {
                ...
                task1_content: currentAnswers['writing_task1'] || "(No submission)", task1_feedback: generateWritingFeedbackHTML(aiResultTask1, 'task1'),
    ...
    useEffect(() => {
        if (!isTestStarted || showResult || loadingData) return;
        ...
        return () => clearInterval(timer);
    }, [isTestStarted, showResult, isSubmitted, loadingData]);
    ...
    if (!testData) {
        return (
            <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f4f7fc' }}>
                ...
                <button onClick={() => navigate('/dashboard')} style={{ background: '#002554', color: 'white', padding: '10px 20px', borderRadius: '6px', border: 'none', cursor: 'pointer', marginTop: '15px' }}>Quay lại Trang chủ</button>
            </div>
        );
    }
        ...
        return (
            <div className="result-screen scrollable">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', margin: '30px auto' }}>
                    <button className="btn-home" style={{ margin: 0 }} onClick={() => navigate(`/test-menu/${testId}`)}>
            ...
            <div className="test-main-content">
                {skill === 'listening' && (
                    <div className="listening-container">
                        <audio 
                            ref={audioRef} 
                            src={testData?.sections?.listening?.audioSrc} 
                            style={{ display: 'none' }} 
                        ...
                        {showListeningStart && (
                            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(255,255,255,0.98)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '8px' }}>
                                <i className="fa-solid fa-headphones" style={{ fontSize: '4.5rem', color: '#002554', marginBottom: '20px' }}></i>
                                <h2 style={{ color: '#002554', marginBottom: '10px', fontSize: '2rem' }}>BÀI THI LISTENING</h2>
                                ...
                                <button 
                                    ...
                                    style={{ background: '#002554', color: 'white', padding: '15px 40px', fontSize: '1.2rem', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', transition: '0.2s' }} 
